...
'use strict';
angular.module('matroshkiApp').controller('studioVideoGalleriesCtrl', ["$scope", "galleryService", "mediaService", "appSettings", "earningService", function ($scope, galleryService, mediaService, appSettings, earningService) {
    ...
    $scope.listGalaryInit = function (id) {
      $scope.modelId = id;
      const options = {
        page: $scope.lastPage, orderBy: $scope.orderBy, sort: $scope.sort, limit: $scope.perPage, type: 'video', modelId: $scope.modelId
      };
...
//delete Gallery
    ...
    $(window).scroll(function () {
      if ($(window).scrollTop() == $(document).height() - $(window).height() && $scope.loadMoreInfinite) {
        galleryService.findMyGalleries({page: $scope.lastPage, orderBy: $scope.orderBy, sort: $scope.sort, limit: $scope.perPage, type: 'video', modelId: $scope.modelId}).success(function (data) {
