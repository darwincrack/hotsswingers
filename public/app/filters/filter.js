angular.module('matroshkiApp')
        ...
        .filter('removeSpaces', function () {
            return function (string) {
                ...
                .filter('estadoGalerias', function () {
            return function (d,lang) {
                var txt = "";
...
//            var date = new Date(date);
        ...
        //https://gist.github.com/rodyhaddad/5896883
        //time: the time
        //local: compared to what time? default: now
        //raw: wheter you want in a format of "5 minutes ago", or "5 minutes"
        return function (time, lang, local, raw ) {
...
if(lang=='es'){
      ...
      if (offset <= MINUTE)              span = [ '', raw ? 'ahora' : 'menos de un minuto' ];
            ...
            else if (offset < (DAY * 7))       span = [ Math.round(Math.abs(offset / DAY)), 'día' ];
            ...
            return (time <= local) ?  ' hace ' + span : 'hace ' + span;
  ...
  if (offset <= MINUTE)              span = [ '', raw ? 'now' : 'less than a minute' ];
            ...
            return (time <= local) ? span + ' ago' : 'ago ' + span;
          ...
          if (offset <= MINUTE)              span = [ '', raw ? 'ahora' : 'menos de un minuto' ];
            ...
            else if (offset < (HOUR * 24))     span = [ Math.round(Math.abs(offset / HOUR)), 'hr'+lang ];
            else if (offset < (DAY * 7))       span = [ Math.round(Math.abs(offset / DAY)), 'día' ];
            else if (offset < (WEEK * 52))     span = [ Math.round(Math.abs(offset / WEEK)), 'semana' ];
            else if (offset < (YEAR * 10))     span = [ Math.round(Math.abs(offset / YEAR)), 'año' ];
            else if (offset < (DECADE * 100))  span = [ Math.round(Math.abs(offset / DECADE)), 'decada' ];
            ...
            return (time <= local) ?  ' hace ' + span : 'ago ' + span;
