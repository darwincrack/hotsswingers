...
class OssnChat extends OssnMessages {
		...
		public static function AllNew() {
				...
				foreach($friends as $friend) {
						...
						$vars['name']   = $friend->username;
						$vars['icon']   = ossn_getimagenes($friend->guid,'small');
		...
		public function getOnlineFriends($user, $intervals = 100) {
				...
				$friend_guids =  str_replace(",,",",",$friend_guids);
				$friend_guids = ltrim($friend_guids, ',');
				$time             = time();
				$params['from']   = 'users';
				$params['wheres'] = array(
						"last_activity > {$time} - {$intervals} AND id IN ({$friend_guids})"
				);
				$friends = false;
