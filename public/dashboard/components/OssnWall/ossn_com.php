...
function ossn_wall() {
		...
		//hooks
		ossn_add_hook('notification:view', 'publish:post', 'ossn_likes_post_notifiation');
...
function ossn_friend_picker() {
		...
		foreach($friends as $users) {
			$first_name_start = mb_substr($users->firstName, 0, $search_len, 'UTF-8');
			if($first_name_start == $search_for || $first_name_start == $search_For) {
				$p['firstName'] = $users->firstName;
				$p['lastName']  = $users->lastName;
				$p['imageurl']   = ossn_getimagenes($users->guid,'small');
				$p['id']         = $users->guid;
				$usera[]         = $p;
			}
		}
