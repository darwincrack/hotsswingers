...
//darwin
		ossn_register_callback('wall', 'post:created', 'ossn_notification_post_create');
...
//darwin
		ossn_add_hook('notification:add', 'publish:post', 'ossn_notificaiton_create_post_hook');
...
function ossn_notification_post_create($type, $ctype, $params) {


//$type, $poster_guid, $subject_guid, $item_guid = NULL, $notification_owner = ''
...
$notification = new OssnNotifications;
		...
		foreach($friends as $users) {
			...
			$notification->add('publish:post', ossn_loggedin_user()->guid , $params['object_guid'],0,$users->guid);	
		}
...
function ossn_notificaiton_create_post_hook($hook, $type, $return, $params) {
