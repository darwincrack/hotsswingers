...
function ossn_messages() {
		...
		if(ossn_isLoggedin()) {
				...
				ossn_register_sections_menu('newsfeed', array(
						'name' => 'lives',
						'text' => 'lives',
						'url' => ossn_getbaseurl('lives'),
						'parent' => 'links',
						'icon' => $icon
				));


				if(ossn_getrole()=='model'){


					$icon          = ossn_site_url('components/OssnProfile/images/friends.png');
					ossn_register_sections_menu('newsfeed', array(
							'name' => 'broadcast',
							'text' => ossn_print('basic:broadcast'),
							'url' => ossn_getbaseurl('models/live'),
							'parent' => 'links',
							'icon' => $icon
					));
...
$icon          = ossn_site_url('components/OssnProfile/images/friends.png');
					ossn_register_sections_menu('newsfeed', array(
							'name' => 'quienestadonde',
							'text' => ossn_print('basic:quienestadonde'),
							'url' => ossn_getbaseurl('travel'),
							'parent' => 'links',
							'icon' => $icon
					));
...
function ossn_messages_page($pages) {
		...
		switch($page) {
				...
				case 'xhr':
						switch($pages[1]) {
								case 'recent':
										...
										if($a ==1 and ossn_loggedin_user()->role == "admin"){
								...
								case 'with':
										...
										if(!empty($guid)) {
												...
												$OssnMessages->markViewed($user->guid,$user_login);
												$params['data']  = $OssnMessages->getWith($user_login, $user->guid);
												$params['count'] = $OssnMessages->getWith($user_login, $user->guid, true);
												$params['user']  = $user;
												$params['admin'] = $a;
												$params['user_login']  = $u;
				...
				if(ossn_loggedin_user()->role == "admin"){

					$params['data'] = $OssnMessages->fullUserChat();

					$title   = ossn_print('messages');
 					$contents['content'] = ossn_plugin_view('messages/admin/fulluserchat',$params);

				}else{

					$contents['content'] = ossn_plugin_view('messages/admin/sinpermisos');
				}
				...
				if(ossn_loggedin_user()->role != "admin"){
					...
					$contents['content'] = ossn_plugin_view('messages/admin/sinpermisos');
						...
						$params['recent'] = $OssnMessages->recentChat($guid);
						$params['count_recent']  = $OssnMessages->recentChat($guid, true);
						$active           = $params['recent'][0];
						if(isset($active->message_to) && $active->message_to == $guid) {
								$getuser = $active->message_from;
						}
						if(isset($active->message_from) && $active->message_from == $guid) {
								$getuser = $active->message_to;
						}
						if(isset($getuser)) {
								$user = ossn_user_by_guid($getuser);
								$OssnMessages->markViewed($getuser, $guid);
								$params['data']   = $OssnMessages->getWith($guid, $getuser);
								$params['countm'] = $OssnMessages->getWith($guid, $getuser, true);
								$params['user']   = $user;
						}
				...
				case 'userchatsolo':
					...
					$contents['content'] = ossn_plugin_view('messages/admin/sinpermisos');
						...
						if(!empty($username)) {
								...
								$OssnMessages->markViewed($user->guid, $loggedin_guid);
								$params['data']  = $OssnMessages->getWith($loggedin_guid, $user->guid);
								$params['count'] = $OssnMessages->getWith($loggedin_guid, $user->guid, true);
								$params['user']  = $user;
								
								$loggedin_guid    = $loggedin_guid;
								$params['recent'] = $OssnMessages->recentChat($loggedin_guid);
								$params['count_recent']  = $OssnMessages->recentChat($loggedin_guid, true);

								$params['admin'] = true;
								$params['guid'] = $loggedin_guid;
...
function linkify_chat($text) {
    ...
    $url_replace = '$1$4$7$10$13<a href="$2$5$8$11$14" \\1 target="_blank" rel="nofollow">$2$5$8$11$14</a>$3$6$9$12';
