...
class OssnMessages extends OssnEntities {
		...
		public function fullUserChat() {


				$database = new OssnDatabase;

				$database->statement("( SELECT  users.id
										FROM
										  users
										INNER JOIN ossn_messages m1
										ON users.guid = m1.message_from
										     )
										UNION DISTINCT   (
										SELECT users.id
										FROM
										  users
										INNER JOIN ossn_messages m2
										ON users.guid = m2.message_to
				...
				/*$params['from']   = 'users';
				$params['joins'][] = "inner join ossn_messages m1 on users.guid = m1.message_from";
				$params['joins'][] = "left join ossn_messages m2 on users.guid = m2.message_to";
				$params['group_by'] = 'users.guid';
