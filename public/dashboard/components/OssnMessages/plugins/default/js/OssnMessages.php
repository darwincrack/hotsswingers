...
$(document).ready(function() {
	...
	$('.ossn-messages .messages-recent .messages-from').scroll(function() {
		if ($('.ossn-pagination').visibleInScroll().isVisible) {
			...
			var a = msg_window.attr('data-a');
			var u = msg_window.attr('data-u');
			...
			if ($next) {
				...
				if ($.inArray($url, $calledOnce) == -1 && $offset > 0) {
					...
					Ossn.PostRequest({
						url: Ossn.site_url + 'messages/xhr/recent' + $url + '&a=' + a + '&u='+ u,
						beforeSend: function() {
...
$(document).ready(function(e) {
	...
	msg_window.scroll(function(event) {
		...
		if (scroll_pos >= old_scroll_pos && offset > old_offset && offset <= last_offset) {
			...
			$a = msg_window.attr('data-a');
			$u = msg_window.attr('data-u');
			Ossn.PostRequest({

				url: Ossn.site_url + 'messages/xhr/with' + $url + '&guid=' + $user_guid + '&a=' + $a + '&u=' + $u,
				beforeSend: function() {
...
Ossn.RegisterStartupFunction(function() {
    $(document).ready(function() {
		$('body').on('click', '.ossn-message-delete', function(e){
				...
				Ossn.MessageBox('messages/delete?id=' + id+'&a='+a+'&u='+u);
		});
		Ossn.ajaxRequest({
