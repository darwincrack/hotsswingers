...
if($params['type']=='group:join:approve'){

$vars['joins'] = array(
								'JOIN users as u ON r.relation_to=u.guid
								JOIN users AS us ON r.relation_from = us.guid'
						);
}
		...
		if(isset($params['to']) && !empty($params['to'])) {
				if(isset($params['inverse'])) {
						$vars['joins'] = array(
								...
								JOIN users as u ON r1.relation_to=u.guid
								JOIN users AS us ON r1.relation_from = us.guid'
						);
...
function ossn_localidades() {
		$buscar           = new OssnDatabase;
		$params['from']   = 'countries';
		return $buscar->select($params, true);

		//return false;
}

function ossn_preferenciades() {
		$buscar           = new OssnDatabase;
		$params['from']   = 'Preferencias';
		return $buscar->select($params, true);

		//return false;
}

function ossn_misgustos() {
		$buscar           = new OssnDatabase;
		$params['from']   = 'gustos';
		return $buscar->select($params, true);
