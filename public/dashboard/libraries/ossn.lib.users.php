...
function ossn_getbaseurl($url = ''){
	$user = forceObject($_SESSION['OSSN_USER']);
			
			return $user->base_url.'/'.$url;
}

function ossn_sitelogo(){
	    $site = new OssnSite;
		return $site->getSitelogo();
}

function ossn_sitenamev(){
	    $site = new OssnSite;
		return $site->getSitename();
}

function ossn_getsettingsite(){
	 $site = new OssnSite;
		return $site->getSettingsite();
}


function ossn_getinfouser($userid){
	 $getuser = new OssnUser;
		return $getuser->getInfouser($userid);
}

function ossn_getpreferenciasuser($userid){
	 $getuser = new OssnUser;
		return $getuser->getPreferenciasuser($userid);
}

function ossn_getgustosuser($userid){
	 $getuser = new OssnUser;
		return $getuser->getGustosuser($userid);
}

function ossn_getallinfouser($userid){
	 $getuser = new OssnUser;
		return $getuser->getAllInfoUser($userid);
}



function ossn_getimagenes($userid, $type = false){
	    $userv = new OssnUser;
		$avatarMeta = $userv->getImagenes($userid);

		if ($avatarMeta==""){
			return ossn_getbaseurl('images/upload/member/memberprofile.jpg');
		}

		$meta = unserialize($avatarMeta);

		if($type=='small'){
			$avatarimg= $meta['imageSmall'];

		}elseif($type=='medium'){
			$avatarimg= $meta['imageMedium'];

		}
		elseif($type=='large'){
			$avatarimg= $meta['imageLarge'];
    ...
    /*if ($key && isset($meta[$key])) {
      ...
      return BASE_URL . $meta[IMAGE_SMALL];
		...
		return ossn_getbaseurl($avatarimg);
}
...
function ossn_localizaciones() {


	return $localidades[] = ossn_localidades();

}

function ossn_preferencias() {


	return $preferencias[] = ossn_preferenciades();

}

function ossn_gustos() {


	return $preferencias[] = ossn_misgustos();
