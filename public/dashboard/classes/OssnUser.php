...
class OssnUser extends OssnEntities {
		...
		public function Login() {
				...
				$user->last_login = 1600912483;
				$user->last_activity = 1600912573;
				$user->activation = "";
				$user->time_created = $user->createdAt;
				$user->fullname = $user->firstName." ".$user->lastName;
				...
				unset($user->stateName);
				unset($user->deviceToken);
				unset($user->mobilePhone);
				unset($user->minPayment);
				unset($user->payoneer);
				unset($user->paypal);
				unset($user->bankAccount);
				unset($user->emailVerified);
				unset($user->emailVerifyToken);
				...
				unset($user->suspendReason);
				unset($user->location_id);
				unset($user->bio);
				unset($user->userMeta);
				unset($user->userSettings);
				unset($user->is_social);
				unset($user->updatedAt);
				unset($user->bankTransferOptions);
				unset($user->directDeposit);
				unset($user->paxum);
				unset($user->autoApprovePayment);
		...
		public function searchUsers(array $params = array()) {
				...
				if(!empty($options['keyword'])) {
						$wheres[] = "(CONCAT(u.firstName, ' ', u.lastName) LIKE '%{$options['keyword']}%' OR
				...
				if(isset($options['entities_pairs']) && is_array($options['entities_pairs'])) {
						foreach($options['entities_pairs'] as $key => $pair) {
								...
								if(!empty($pair['name']) && isset($pair['value']) && !empty($operand)) {
										...
										$params['joins'][] = "JOIN ossn_entities as e{$key} ON e{$key}.owner_guid=u.id";
				...
				if(!empty($options['condicionaldos'])){
					$wheres[] = $options['condicionaldos']." and u.createdAt IS NOT NULL";
				}
				else
				{

					$wheres[] = "u.createdAt IS NOT NULL";
				...
				if(!empty($options['condicionalseis'])){

					$from .= "INNER JOIN gustos_personas ON gustos_personas.users_id = u.id ";
				}

				if(!empty($options['condicionalcinco'])){

					if(!empty($from))
					{

						$from .= "INNER JOIN PreferenciasPersonas ON PreferenciasPersonas.users_id = u.id ";
		...
		public function getUnvalidatedUSERS($search = '', $count = false) {
				...
				if(empty($search)) {
						...
						$params['wheres'] = array(
								"activation <> ''",
								"CONCAT(firstName, ' ', lastName) LIKE '%$search%' AND activation <> '' OR
		...
		public function countByYearMonth() {
				...
				$params['from']     = "users";
				$params['params']   = array(
						"DATE_FORMAT(FROM_UNIXTIME(createdAt), '%Y') AS year",
						"DATE_FORMAT(FROM_UNIXTIME(createdAt) , '%m') AS month",
						"DATE_FORMAT(FROM_UNIXTIME(createdAt),'%Y%m') AS YM",
						"COUNT(id) AS total"
				);
		...
		public function onlineByGender($gender = 'male', $count = false, $intervals = 100) {
				...
				$params['joins'][] = "JOIN ossn_entities as e ON e.owner_guid=u.id";
		...
		public function getImagenes($userid){
				$params['from']  = 'users';
				$params['wheres'] = array(
								"id = '{$userid}'"
						);
		...
		public function getPreferenciasuser($userid){


			$this->statement("select Preferencias.* from Preferencias
				  inner join PreferenciasPersonas
				  on Preferencias.Preferencias_id=PreferenciasPersonas.Preferencias_id
				  inner join users
				  on
				  PreferenciasPersonas.users_id =  users.id
	...
	public function getGustosuser($userid){


			$this->statement("SELECT gustos.*
						FROM
						  gustos
						INNER JOIN gustos_personas
						ON gustos.gustos_id = gustos_personas.gustos_id
						INNER JOIN users
						ON
						gustos_personas.users_id = users.id
						WHERE
						  users.id = '{$userid}'");
				$this->execute();
				$data = $this->fetch(true);
