  ...
  return {
    ...
    getPerformer: function getPerformer() {
      return $http.get(appSettings.BASE_URL + 'api/v1/profile/performer');
        ...
        Participantes: function (id) {
      return $http.get(appSettings.BASE_URL + 'api/v1/participantes/' + id);
    },

    FindParticipantes: function (id) {
      return $http.get(appSettings.BASE_URL + 'api/v1/find-participantes/' + id);
    },

    SetConnectByUser: function (modelId,userConnect) {
      return $http.get(appSettings.BASE_URL + 'api/v1/user/setconnect/' + modelId +'/' + userConnect);
    },
  ...
  return {
    ...
    sendTipTokens: function (roomId, tokens) {
      return  $http({
       ...
       sendTipTokensPrivados: function (roomId, tokens) {
      return  $http({
        method: 'post',
        url: appSettings.BASE_URL + 'api/v1/member/send-tip-tokens-privados/' + roomId,
        data: {
          tokens: tokens
        }
      }).then(function successCallback(response) {
  ...
  return {
    ...
    */
    sendChatMessage: function sendChatMessage(data) {
      ...
      Multiretoactual(data) {
        socket.emit('multireto-actual', data);
      }, 


      onMultiretoactual(cb) {
        cb = cb || angular.noop;
        socket.on('multireto-actual', cb);
      },


      Multiretofinalizar(data) {
        socket.emit('multireto-finalizar', data);
      }, 


      onMultiretofinalizar(cb) {
        cb = cb || angular.noop;
        socket.on('multireto-finalizar', cb);
      },

      Videoprivadoxactivar(data) {
        socket.emit('videoprivadox-activar', data);
      }, 


      onVideoprivadoxactivar(cb) {
        cb = cb || angular.noop;
        socket.on('videoprivadox-activar', cb);
      },


    onVideoprivadoxactivarmsg(cb) {
        cb = cb || angular.noop;
        socket.on('videoprivadox-activar', cb);
      },


      Videoprivadoxdesactivar(data) {
        socket.emit('videoprivadox-desactivar', data);
      }, 


      onVideoprivadoxdesactivar(cb) {
        cb = cb || angular.noop;
        socket.on('videoprivadox-desactivar', cb);
      },
      ...
      * send new-chat-message event to server
      */
      sendChatMessageMultiretos(data) {
        socket.emit('new-chat-message', data);
      },
    /**
    ...
    */
    ...
    onReceiveTip: function onReceiveTip(cb) {
      ...
      * send send-tip event to server
      */
      sendTipMultiretos(data) {
        socket.emit('goal-tip-multiretos', data);
      },

      /**
       * Event for send tip callback
       */
      onreciveTipMultiretos(cb){
        cb = cb || angular.noop;
        socket.on('goal-tip-multiretos', cb);
      },
   ...
   Multiretostip(cb){
    ...
    emit: function emit(event, data, cb) {
      ...
      */
      ...
      onInitRuleta(cb){
        cb = cb || angular.noop;
        socket.on('init-ruleta', cb);
      },



   onInitRuletachart(cb){
        cb = cb || angular.noop;
        socket.on('init-ruleta', cb);
      },
...
/**
      * send send-tip event to server
      */
      FinalizarRuleta(data) {
        socket.emit('finalizar-ruleta', data);
      },

      /**
       * Event for send tip callback
       */
      onFinalizarRuleta(cb){
        cb = cb || angular.noop;
        socket.on('finalizar-ruleta', cb);
      }
...
angular.module('matroshkiApp').factory('onlineService', ['$http', 'appSettings', 'commonHelper', function ($http, appSettings, commonHelper) {
  return {
    ...
    getTravel: function (filter,maps,fecha) {
      return $http.get(appSettings.BASE_URL + 'api/v1/travel?filter=' + filter + '&maps=' + maps + '&fecha=' + fecha);
    },
