...
class MediaController extends Controller {
  ...
  public function getVideosByGallery($slug, Request $req){
    ...
    $galleryNotExistMediaControl = trans('messages.galleryNotExistMediaControl');
    ...
    if(!$gallery){
        return Redirect('videos')->with('msgError', $galleryNotExistMediaControl);
    }
  ...
  public function getVideoDetail($slug) {
    ...
    if($userData->role == 'admin'){
        $video = VideoModel::select('videos.*', DB::raw("(select ap.mediaMeta from attachment ap where ap.media_type = 'poster' AND ap.id = videos.poster limit 1) as posterData"), 'a2.mediaMeta as videoMeta', 'u.username')
          ->where('videos.status', VideoModel::ACTIVE)
         /*->join('attachment as a', 'a.id', '=', 'videos.poster')
      ...
      $video = VideoModel::select('videos.*', DB::raw("(select ap.mediaMeta from attachment ap where ap.media_type = 'poster' AND ap.id = videos.poster limit 1) as posterData"), 'a2.mediaMeta as videoMeta', 'u.username', DB::raw("(SELECT p.id FROM paymenttokens p where p.itemId=videos.id AND p.item='" . PaymentTokensModel::ITEM_VIDEO . "' AND p.status <> '".PaymentTokensModel::STATUS_REJECT."' AND p.ownerId={$ownerId}) AS bought"))
        ->where('videos.status', VideoModel::ACTIVE)
    ...
    if (!$video) {
      return Redirect('models/dashboard/media/videos')->with('msgError', 'Video not exist or not available.');
    }
