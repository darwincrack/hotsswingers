...
class PerformerPayoutRequest extends Model {
  ...
  public function save(array $options = array()) {
    ...
    $newpayoutrequestfromperformer = trans('messages.newpayoutrequestfromperformer');
    $request = trans('messages.request');
    ...
    if ($isNew) {
      ...
      Mail::send('email.new_request', [
        'performer' => $performer,
        'request' => $this
      ], function($message) use ($performer,  $sendTo, $newpayoutrequestfromperformer){
        $siteName = app('settings')->siteName;
        $message
          ->from(env('FROM_EMAIL') , app('settings')->siteName)
          ->to($sendTo)
          ->subject(trans('messages.newpayoutrequestfromperformer')." #{$performer->id} - {$performer->user->username} | {$siteName}");
      });
    } else {
      ...
      if ($isStatusChanged) {
        ...
        Mail::send('email.update_status', [
          ...
          $message
            ...
            ->subject(trans('messages.request')." #{$request->id} ".trans('messages.hasbeenupdated')." | {$siteName}");
        });
      }
    }
