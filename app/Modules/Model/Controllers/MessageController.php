...
class MessageController extends Controller {
  ...
  public function sendMessage(Request $get) {
    ...
    if ($userOne !== $userTwo) {
      ...
      return redirect('models/dashboard/messages/new-thread')->with('msgInfo', $messagehasbeensentto2 . $get->username . ' ');
    } else {
      return redirect()->back()->with('msgError', $cannotSendToYourSelf);
    }
  }
...
//      ->whereRaw('(messageconversation.userOne =' . $userLogin->id . ' or messageconversation.userTwo =' . $userLogin->id . ') ')
  ...
  public function getPrivateMessage($threadId) {
    ...
    if (empty($ThreadMessage)) {
      return redirect('models/dashboard/messages')->with('msgError', $messageThreadIDNotFound);
    }
    $userLogin = AppSession::getLoginData();
...
//    $this->dispatch($job);
  ...
  public function postRelayPrivateMessage(Request $get, $threadId) {
    ...
    $pleaseloginwithmodelrole = trans('messages.conversationNotFound')."!";
    ...
    if (empty($getMessageThread)) {
      return redirect('models/dashboard/messages')->with('msgError', $pleaseloginwithmodelrole);
    }
    $messageReplay = new MessageReplyModel();
