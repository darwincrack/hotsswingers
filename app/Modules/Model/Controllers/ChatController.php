<?php
  ...
  class ChatController extends Controller {
      ...
      public function privateChat($memberId, Request $req) {
        ...
        $roomInitError = trans('messages.roomInitError');
        $userNotFound = trans('messages.userNotFound');
          ...
          if (!$req->has('vr')) {
              return redirect('models/live')->with('msgError', $roomInitError);
          }

          //get member data
          $member = User::where('id', '=', $memberId)->first();
          if (!$member) {
              return redirect('models/live')->with('msgError', $userNotFound);
          }
