...
\App::setLocale(session('lang'));
  ...
  public function getGalleryView($username, $id) {
    ...
    $requestNotAllowed = trans('messages.requestNotAllowed')."!";
    ...
    if (!empty($getModel)) {
      ...
      return redirect('community')->with('msgError', $requestNotAllowed);
    }
  }
...
/**
  ...
  **/
  public function getVideoView($username,$id){
    ...
    $requestNotAllowed = trans('messages.requestNotAllowed')."!";
    $galleryNotFound = trans('messages.galleryNotFound')."!";
    ...
    if (!empty($getModel)) {
      ...
      if (!empty($checkGalleryExisting)) {
        ...
        return redirect('profile/' . $getModel->username . '?view=videos')->with('msgError', $galleryNotFound);
      }
    } else {
      return redirect('community')->with('msgError', $requestNotAllowed);
    }
  ...
  **/
  public function postVideoItem(Request $get){
    ...
    $videoNotFound = trans('messages.videoNotFound')."!";
    $requestNotAllowed = trans('messages.requestNotAllowed');
    ...
    if(\Request::ajax()){
      ...
      return redirect()->back()->with('msgError',$requestNotAllowed);
    }
  }
  ...
  **/
  public function postLikeVideoItem(Request $get){
    ...
    $youLikedIt = trans('messages.youLikedIt');
    $likeModelSuccessful = trans('messages.likeModelSuccessful');
    $requestTimeOut = trans('messages.requestTimeOut');
    $requestNotAllowed = trans('messages.requestNotAllowed');
