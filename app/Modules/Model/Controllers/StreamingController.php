  ...
  public function goLive(Request $get) {
    ...
    if(!$model || $model->role != UserModel::ROLE_MODEL){
      ...
      if($model->accountStatus == UserModel::ACCOUNT_WAITING){
        ...
        }else if($document->contrato == 0 or $document->contrato == 1){
            return redirect("models/dashboard/account-settings?action=documents")->with('msgError', "La firma de su Acuerdo independiente de emisor/a aun no ha sido aprobado");
      ...
      }else if($model->accountStatus == UserModel::ACCOUNT_ACTIVE){
