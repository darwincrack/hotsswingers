...
//      var_dump($attribute,$value, $parameters);
  ...
  public function approveAccount($id) {
    ...
    if ($user->accountStatus == UserModel::ACCOUNT_ACTIVE and  $document->contrato == 2) {
    ...
    if ($user->save()) {
      if ($userStatus == UserModel::ACCOUNT_WAITING or $document->contrato == 1) {
...
//check connect table and user manage
 ...
 $genero = array('male'=>'Hombre','female'=>'Mujer', 'transgender' => 'transgenero','pareja' => 'pareja');
  ...
  public function getConversaciones(){
          ...
          (SELECT message_from  as id
                   FROM
                     ossn_messages

                   group by message_from  ORDER BY
            time) union (SELECT message_to  as id
                   FROM
                     ossn_messages
                   group by message_to ORDER BY
            time)"));
          ...
          $query = UserModel::select('users.id','users.username')
          ->whereIn('id', $arr);
          ...
          $grid = new Grid(
            (new GridConfig)
              ...
              ->setColumns([
                ...
                ->setLabel('Acciones')
                ->setCallback(function ($val) {
                  return '<a class="btn btn-success btn-sm" target = "_blank" href="'.URL('dashboard/messages/userchat/'.$val).'">Ver conversaciones</a>';
                  })
                ->setSortable(false)
                ,
              ])
      ...
      \App::setLocale(session('lang'));
          ...
          return view('Admin::admin_conversaciones')->with('grid', $grid->render());
    }
