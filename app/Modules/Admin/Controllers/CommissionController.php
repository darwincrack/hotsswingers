    ...
    $grid = new Grid(
      (new GridConfig)
        ...
        ->setColumns([
          ...
          ->setLabel('Acciones')
          ->setCallback(function ($val) {
            return '<a class="btn btn-warning btn-sm" href="'.URL('admin/manager/commission/edit/'.$val).'">Editar</a>';
            })
          ->setSortable(false)
          ,
        ])
      ...
      (new GridConfig)
        ...
        ->setColumns([
         ...
         /* ,
          (new FieldConfig)
          ->setName('action')
          ->setLabel('Filtrar modelos de Estudio')
          ->setCallback(function ($val) {
            return '<a class="btn btn-warning btn-sm" href="'.URL('admin/manager/commission/edit/'.$val).'">Edit</a>';
            })
          ->setSortable(false)
...
\App::setLocale(session('lang'));
  ...
  public function updateCommission($id) {
    ...
    $commission = EarningSettingModel::earningDetail($id);
    if (!$commission) {
      return Redirect::to('admin/manager/commission')->with('msgError', 'Comisión no existe.');
    }
    ...
    $earning->performerSiteMember = Input::get('referredMember');
    $earning->otherMember = Input::get('referredMember');
    if ($earning->save()) {
      return Back()->with('msgInfo', 'La comisión se actualizó con éxito.');
    }
    return Back()->withInput()->with('msgError', 'Error de actualización del sistema.');
  }
