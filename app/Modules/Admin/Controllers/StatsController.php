...
class StatsController  extends Controller{
   ...
   (new FieldConfig)
      ...
      ->setCallback(function ($val, $row) {
        $item = $row->getSrc();
        $earning = PaymentTokensModel::getMyEarned($item, 'all', PaymentTokensModel::ITEM_REFERIDOS);
        return ($earning->totalTokens)?$earning->totalTokens*$GLOBALS['conversionRateForGrid']:'';
      }),
      (new FieldConfig)
      ->setName('tip')
    ...
    if(!env('DISABLE_EDIT_ADMIN') || $adminData->isSuperAdmin) {
      ...
      ->setCallback(function ($val, $row) {
          $item = $row->getSrc();
          $url = "<a title='Reset payment' class=\"btn btn-danger btn-sm\" onclick=\"return confirm('¿Está seguro de que desea restablecer el pago de esta cuenta?')\" href='" . URL('admin/stats/reset-earning/' . $val) . "'>Reset</a>";
          return $url;
        })
      ->setSortable(false);
    }
  ...
  public function resetEarning($userId) {
    ...
    if(!$user || !$performer){
      return Back()->with('msgError', 'Usuario no encontrado');
    }
   /* if($user) {
      $user->tokens = 0 ;
      $user->save();
    ...
    DB::table('paymenttokens')->where('modelId', '=', $userId)->where('item', '=', 'image')->delete();
    DB::table('paymenttokens')->where('modelId', '=', $userId)->where('item', '=', 'video')->delete();
    DB::table('performer_payout_requests')->where('performerId', '=', $performer->id)->delete();
    DB::table('earnings')->where('payTo', '=', $userId)->delete();
    return Back()->with('msgInfo', 'Restablecer con éxito!');
  }
}
