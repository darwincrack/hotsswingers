...
//        ->whereRaw("u.id not in (select bl.userId from blacklist bl where locker={$modelId}) and bl.lock='yes'")
  ...
  public function deleteMessages(){
     ...
     $deleteChatMessageSuccessful = trans('messages.deleteChatMessageSuccessful');
     $systemErrorMemberLogin = trans('messages.systemErrorMemberLogin');


      $userData = AppSession::getLoginData();
      if(!$userData || ($userData->role != UserModel::ROLE_ADMIN && $userData->role != UserModel::ROLE_SUPERADMIN)){
          return response()->json(array('success'=>false, 'message'=>\Lang::get('messages.youHaveNoPermission')));
      }
    ...
    if ($chatMessages) {
      return response()->json(array('success' => true, 'message' => $deleteChatMessageSuccessful));
    }
      return response()->json(array('success' => false, 'error' => $systemErrorMemberLogin));
