...
//    }
  ...
  public function setMemberFavorite() {
     ...
     $modelnotexist = trans('messages.modelnotexist');
     $loginToAddFavoriteList = trans('messages.loginToAddFavoriteList');
     $systemErrorMemberLogin = trans('messages.systemErrorMemberLogin');


    if (!Input::has('model')) {
      return Response()->json(['success' => false, 'message' => $modelnotexist]);
    }
    $userData = AppSession::getLoginData();
    if (!$userData) {
      return Response()->json(['success' => false, 'message' => $loginToAddFavoriteList]);
    }
    //check model
    ...
    return Response()->json(['success' => false, 'message' => $systemErrorMemberLogin]);
  }
