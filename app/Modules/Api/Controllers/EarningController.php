...
//    $query .= " LIMIT {$skip}, {$take}";
  ...
  public function getDetail(Request $req) {
    ...
    if ($req->get('by') == 'day') {
      ...
      //     ->get();
       return EarningModel::select('earnings.id', 'earnings.tokens', 'earnings.status', 'earnings.type', 'u.username', DB::raw('u.username as fromName'), 'earnings.item', 'earnings.itemId', 'earnings.createdAt')
          ->join('users as u', 'u.id', '=', 'earnings.payFrom')
          ->where("earnings.createdAt", 'like', $day . '%')
          ->where('earnings.payTo', $userData->id)
          ->orderBy('earnings.createdAt', 'desc')
      ...
      $detail = EarningModel::select('earnings.id', 'earnings.tokens', 'earnings.status', 'earnings.type', 'u.username',  DB::raw('u.username as fromName'), 'earnings.item', DB::raw('u1.username as modelName'), 'earnings.itemId', 'earnings.createdAt')
        ->join('users as u', 'u.id', '=', 'earnings.payFrom')
        ->join('paymenttokens as p', 'p.id', '=', 'earnings.itemId')
        ->join('users as u1', 'u1.id', '=', 'p.itemId')
  ...
  public function getEarningByItem($item, $id) {
    ...
    $earningDetailNotFound = trans('messages.earningDetailNotFound');
    ...
    return Response()->json(['success' => false, 'message' => $earningDetailNotFound]);
  }
