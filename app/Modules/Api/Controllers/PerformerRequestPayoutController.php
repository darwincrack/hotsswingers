  ...
  public function getComments(Request $req, $id) {
    ...
    if (!$request) {
      return Response()->json([
        'success' => false,
        'data' => [
          'message' =>trans('messages.requestNotFoundMemberLogin')
        ]
      ]);
    }
...
//    }
  ...
  public function updateStatus(Request $req, $id) {
    ...
    if(!$userData || ($userData && $userData->role != 'studio' && $userData->role != 'admin')){
      return response(['message'=> trans('messages.Youdoesnothavepermissiononthisection')], 403)->header('Content-Type', 'application/json');  
    }
    ...
    if (!$request) {
      return response(['message'=>trans('messages.requestNotFoundMemberLogin')], 403)->header('Content-Type', 'application/json');  
    }
