...
class PerformerChatController extends Controller {
  ...
  public function updatePerformerChat($modelId) {
    ...
    if (!$userData || ($userData->role != UserModel::ROLE_MODEL)) {
      return Response()->json(['success' => false, 'message' => \Lang::get('messages.pleaseloginwithmodelrole')]);
    }
    ...
    if ($settings->save()) {
      return Response()->json(['success' => true, 'message' => \Lang::get('messages.chatSettingsHaveBeenSaved')]);
    }
    return Response()->json(['success' => false, 'message' => \Lang::get('messages.systemError')]);
  }
