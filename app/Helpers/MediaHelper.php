...
class MediaHelper {
	...
	public static function upload($userData, $uploadFile, $mediaType, $parentId, $modelId){
	    ...
	    if (count($_FILES) === 0 || empty($_FILES['myFiles']['tmp_name'])) {
	      return array('success' => false, 'message' => trans('messages.fileTooLarge') . ini_get("post_max_size"), 'error' => '', 'file' => '');
	    }
	    ...
	    if ($validator->fails()) {
	      ...
	      if ($uploadFile->isValid()) {
	        ...
	        return array('success' => true, 'error' => '', 'message' => trans('messages.uploadSuccessfully'), 'file' => $uploadFile);
	      } else {
	        return array('success' => false, 'error' => trans('messages.uploadFileNotValid'), 'message' => '', 'file' => '');
	      }
	    }
	}
	public static function setProfileImage($userData, $attachmentId){
	    ...
	    if(!AppHelper::is_serialized($avatar)){
	        return array('success' => false, 'message' => trans('messages.imageFormatError'));
	    }
	    $imageMeta = unserialize($avatar);
	    if(!file_exists(public_path($imageMeta[IMAGE_SMALL]))){
	        return array('success' => false, 'message' => trans('messages.imagenotexist'));
	    }
	    ...
	    if($user->save()){
	        ...
	        return array('success' => true, 'message' => trans('messages.defaultprofileimage'));
	    }
	    return array('success' => false, 'message' => trans('messages.cannotChangeProfileImage'));
	}
}